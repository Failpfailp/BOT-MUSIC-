...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const Sort = Queue.Songs.map(async (Song, Position) => `${(Position + 1) === 1 ? `Now Playing: ${Song.Title}` : `${Position + 1} | ${Song.Title.length > 55 ? Song.Title.slice(0, 55) + "..." : Song.Title}`}`).join("\n");
